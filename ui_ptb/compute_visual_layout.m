...
%COMPUTE_VISUAL_LAYOUT 根据视觉角度计算棋盘与按钮的像素布局。
%   流程：获取屏幕物理尺寸 → 视角换算 → 棋盘/按钮矩形 → 安全显示检查
...
board_w_px = config.game.cols * cell_px;   % 棋盘总宽
board_h_px = config.game.rows * cell_px;   % 棋盘总高

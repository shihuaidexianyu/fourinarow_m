...
draw_buttons(ui, struct('start_game', layout.start_button), {'start_game'}, {'START'}, 'start');

if config.ui.show_config_summary
    human_color = ternary(config.game.human_player==1, 'black', 'white');
    agent_color = ternary(config.game.agent_player==1, 'black', 'white');
    first_color = ternary(config.game.first_player==1, 'black', 'white');
    summary = sprintf(['Board: %dx%d (free placement)\n' ...
        'human: %s, agent: %s\n' ...
        'first: %s'], ...
